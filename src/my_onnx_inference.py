...
def run(args):
    ...
    transform = Compose(
        [
            ToTensor(),
            # Resize((1024, 1024)),
            Resize((1536, 1536)),  # Resize the image to a fixed size
            Lambda(lambda x: x.to(get_torch_device())),
            Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5]),
            ConvertImageDtype(torch.float32 if not args.half_float else torch.float16),
        ]
    )
    ...
    for epoch in range(args.all_epochs):
        ...
        if epoch < args.warm_up_epochs:
            ...
            for image_path in tqdm(image_paths):
                ...
                input_tensor = (
                    transform(image).unsqueeze(0).cpu().numpy()
                )  # Add batch dimension
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Inference scripts of DepthPro with ONNX models."
    )
    parser.add_argument(
        "-i",
        "--image-path",
    ...
    parser.add_argument(
        "--onnx-model-path",
        type=Path,
        default="./onnx_weights/model_fp16.onnx",
